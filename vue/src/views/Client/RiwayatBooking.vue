...
const perPage = 5
const selectedStatus = ref('All'); // Tambahkan state untuk filter status
...
const allBookings = computed(() => {
  // Gunakan getter historicalBookings dari store
  return bookingStore.historicalBookings;
});

const bookingCounts = computed(() => {
  const counts = { All: 0, Completed: 0, Declined: 0 };
...
const fetchAndProcessAllBookingExtraData = () => {
  ...
  allBookings.value.forEach((booking) => { // Gunakan allBookings di sini
    ...
    if (booking.status === 'Completed' && serviceId && bookingDate) { // Tetap cek 'Completed' untuk review
...
// Watchers
watch(allBookings, fetchAndProcessAllBookingExtraData, { immediate: true })
watch(() => bookingStore.userReviews, fetchAndProcessAllBookingExtraData, { deep: true })
watch([selectedStatus, selectedDate, searchQuery], () => { // Tambahkan selectedStatus di watcher
  currentPage.value = 1
})
  ...
  <div class="bg-gray-100 min-h-screen p-4">
    ...
    <div class="max-w-4xl mx-auto">
      <div class="mb-6 bg-white rounded-lg shadow p-6">
        <h1 class="text-3xl font-bold text-gray-800 mb-6">Riwayat Booking</h1>
        <div class="flex flex-col sm:flex-row sm:items-center gap-4 text-sm">
          <div class="relative flex-grow sm:flex-grow-0 sm:w-96 order-1 sm:order-1">
            <input
              type="text"
              v-model="searchQuery"
              placeholder="Cari judul, opsi, catatan..."
      ...
      <div>
        ...
        <div v-else-if="filteredBookings.length === 0" class="bg-white rounded-lg shadow p-6 text-center text-gray-600">
          <p class="text-lg">Tidak ada riwayat booking.</p>
          <p class="text-sm mt-2">Coba ubah filter atau buat booking baru.</p>
        </div>
